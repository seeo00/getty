...
const InfoCard = () => {
  ...
  const { detailsData, loading, error } = useSelector((state) => state.detailsR, shallowEqual);
  ...
  const originCountry = originCountryCode ? countryMap[originCountryCode] || '정보 없음' : '정보 없음';
  ...
  const writers =
    detail.created_by && detail.created_by.length > 0
      ? detail.created_by
      : detail.credits && detail.credits.crew
      ? detail.credits.crew.filter((person) => person.job === 'Screenplay' || person.job === 'Writer')
      : [];
  ...
  return (
    <InfoCardContainer key={detail.id}>
      ...
      <TextAndDetailWrapper>
        ...
        <DetailList>
          <li>
            <strong>개요 :</strong> {country ? country : '개요 정보 없음'}, <span>{releaseDate}</span>
          </li>
          <li>
            <strong>감독 :</strong>{' '}
            {directors.length > 0
